...
def inforParser(html):
    ...
    soup = BeautifulSoup(html, 'lxml').find('div',
                                            attrs={'class': 'a-container'})
    if soup == None:
        soup = BeautifulSoup(html, 'lxml')
    try:
        title = soup.find('div', id='title_feature_div').get_text().replace(
            '\r', '').replace('\n', '')
        infor['title'] = title.replace(
            '#energyEfficiencyTitleInformation { font-size: 19px !important;}',
            '')
    except:
        infor['title'] = '--'
    try:
        infor['commentsNumber'] = soup.find(
            'div', id='averageCustomerReviews_feature_div').find(
                'a', id='acrCustomerReviewLink').get_text().replace(
                    '\r', '').replace('\n', '')
    except:
        infor['commentsNumber'] = '--'
    try:
        table = soup.find('div', id='featurebullets_feature_div').find(
            'ul', attrs={'class': 'a-vertical a-spacing-none'}).find_all('li')
        text = ''
        for li in table:
            text += li.get_text() + '\n'
        infor['feature'] = text
    ...
    try:
        try:
            ...
            try:
                style = soup.find('li',
                                  id='SalesRank').find('style').get_text()
            except:
                style = ''
        ...
        infor['Rank'] = '--'
    try:
        infor['Des'] = soup.find('div', id='descriptionAndDetails').find(
            'div', id='productDescription').get_text()
    except:
        infor['Des'] = '--'
    try:
    ...
    date_re = {'uk': 'Datefirstavailable(.*?)\|',
               'de': 'ImAngebotvonAmazon.deseit(.*?)\|',
               'jp': 'Amazon.co.jpでの取り扱い開始日(.*?)\|',
               'com': 'DatefirstavailableatAmazon.com(.*?)\|',
               'fr': 'DatedemiseenlignesurAmazon.fr(.*?)\|',
               'it': 'DisponibilesuAmazon.itapartiredal(.*?)\|',
               'es': 'ProductoenAmazon.esdesde(.*?)\|'}
    try:
        for rel in asin_rel:
            try:
